...
const getLocalData = () => {
  ...
  return {
    token: storedToken,
    exp: storedExp,
    userId: storedId, // Keep as string if it's a UUID
    username: storedName,
  };
};
  ...
  const reducer = (state, action) => {
    switch (action.type) {
      case "LOGIN":
        ...
        localStorage.setItem("userId", userId); // Ensure this is saved correctly
      ...
      case "RETURNING_USER":
        ...
        return { ...state, token: t, userId: u, exp: e, username: n }; // Do not convert userId to number
      default:
        return state;
    }
  };
